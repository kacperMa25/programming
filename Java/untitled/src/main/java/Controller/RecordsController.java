...
public class RecordsController {
    ...
    public void compressZip(String fileName) throws IOException {
        try (FileOutputStream fos = new FileOutputStream(fileName);
             ZipOutputStream zos = new ZipOutputStream(fos)) {

            ZipEntry entry = new ZipEntry("workers.zip");
    ...
    public boolean removeWorker(String pesel){
        return workers.remove(pesel) != null;
    }

    public boolean addWorker(Pracownik worker){
        if (Pracownik.isPeselCorrect(worker.getPesel()) == 1) {
            workers.put(worker.getPesel(), worker);
            return true;
        }
        return false;
